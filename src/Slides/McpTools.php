...
class McpTools
{
	 ...
	 * size) wherever the text is unchanged: only the differing part is rewritten, so the common
	 * start and end keep their styling (a naive clear-and-retype would reset everything). An empty
	 * shape is filled cleanly; an empty string clears the shape. The objectId must be a text-bearing
	 * shape, NOT a table — discover it with slides_get_presentation. For deck-wide substitution use
	 * slides_replace_all_text; to add text without removing what's there use slides_insert_text; to
	 * restyle without changing text use slides_format_text.
	 *
	 * Inline styling: omit styles and the rewritten part inherits the style of the character before
	 * it (Slides API behavior) — editing right after a bold word silently bolds the new text. Pass
	 * styles and the server first clears bold/italic/underline on the rewritten run (so nothing
	 * inherits), then applies exactly the styles you list, matched by literal substring against the
	 * new text; occurrence (zero-based) targets one match of a repeated substring, fontSizePt/color
	 * keep inheriting unless set. The response echoes how many occurrences each substring matched —
	 * 0 means no match (likely a typo), so check it. This is the one-call way to write a styled
	 * bullet block: the full text plus one styles entry per bold keyword.
	 *
	 * Soft line breaks: \v in the text stands for a soft break inside a paragraph (the server
	 * converts it to U+000B); a real newline starts a new paragraph.
	 *
	 * @param string $presentationId  The presentation ID
	 * @param string $objectId  Object ID of the target shape
