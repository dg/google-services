...
- `getTokenDiagnostics()` returns a **non-sensitive** token lifecycle snapshot (never the token values) for diagnosing the intermittent mid-session 401 "Invalid Credentials". Reads `token.json` from disk as ground truth: `locallyExpired` mirrors `Client::isAccessTokenExpired()` math (`created + expires_in - 30 < now`), plus `created`/`expiresIn`/`ageSeconds`/`hasRefreshToken`. Also compares the live client's in-memory token against disk — `memoryMatchesDisk: false` means the lib refreshed the access token in-process but never persisted it (the rejected request then used a value the disk snapshot can't see). Wired into the `onApiError` diagnostic hook in `server.php`
...
- Configured via env vars (the `.mcp.json` host gives them to the process):
  ...
  - `GOOGLE_LOG_DIR` — directory for the auth-failure diagnostic log (default: the token dir)
- Wires an `onApiError` diagnostic hook into the central `McpToolCallGuard` (one hook covers Gmail/Calendar/Slides, since the guard converts every tool's errors). On a Google API exception the hook decides whether it looks auth-related and, if so, appends one JSON line (timestamp, HTTP code, error, `Authenticator::getTokenDiagnostics()` snapshot) to `<logDir>/auth-failures.log`. This is evidence-gathering for the still-unexplained intermittent mid-session 401 "Invalid Credentials". The auth-failure test is **deliberately narrow**: a genuine `401` or an auth-specific message string (`invalid credentials`/`invalid_grant`/`unauthorized`) — **403 is excluded by status code alone** because Gmail/Calendar return 403 for `rateLimitExceeded`/`quotaExceeded`, which would otherwise flood the log. The hook is null-safe (the guard defaults it to `null`) so the success path and tests are unaffected
