...
A PHP library wrapping Google APIs (Gmail, Calendar, Meet) with a more ergonomic API on top of the official `google/apiclient`. Also ships an **MCP server** (stdio, single-user) that exposes Gmail operations plus read-only Calendar listing (`calendar_list_events`, `calendar_list_calendars`) as tools to Claude and other MCP clients.
